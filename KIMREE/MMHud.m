...
NSString * const MMProgressHUDFontNameNormal = @"HelveticaNeue-Light";
    ...
    #else
//        static const BOOL MMProgressHUDFrameDebugModeEnabled = 0;
    #endif
#else
...
- (CGSize)titleLabelSizeForTitleText:(NSString *)titleText {
    ...
    CGFloat lineHeight = [titleText boundingRectWithSize:self.titleLabel.frame.size withTextFont:self.titleLabel.font withLineSpacing:0].height;
    ...
    for (CGFloat targetWidth = MMProgressHUDMinimumWidth; numberOfLines > 2; targetWidth += targetWidthIncrementor) {
        if(targetWidth >= MMProgressHUDMaximumWidth)
            break;
//        titleSize = [titleText sizeWithFont:self.titleLabel.font constrainedToSize:CGSizeMake(targetWidth, 500.f)];//ios7以上不能使用
        NSDictionary *attribute = @{NSFontAttributeName:self.titleLabel.font};
        titleSize = [titleText boundingRectWithSize:self.titleLabel.frame.size options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
...
//                                  constrainedToSize:CGSizeMake(targetWidth, 500.f)];
        ...
        NSDictionary *attribute = @{NSFontAttributeName:self.titleLabel.font};
        statusSize = [self.messageText boundingRectWithSize:self.titleLabel.frame.size options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
    }
    return statusSize;
}
