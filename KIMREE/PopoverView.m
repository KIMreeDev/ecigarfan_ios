...
#define POPOVER_RADIUS 10.0


@interface PopoverView(Private)
-(void)setupViews;
@end
...
-(CGPathRef)createContentPathWithBorderWidth:(CGFloat)borderWidth arrowDirection:(PopoverArrowDirection)direction
{
    CGFloat w = self.bounds.size.width;
    CGFloat h = self.bounds.size.height;
    CGFloat ah = POPOVER_ARROW_HEIGHT; //is the height of the triangle of the arrow
    CGFloat aw = POPOVER_ARROW_BASE/2.0; //is the 1/2 of the base of the arrow
    ...
    else if(direction == PopoverArrowDirectionDown)
    {
        rect.size.width = w - 2*b;
        rect.size.height = h - ah - 2*b;
        rect.origin.x = b;
    ...
    else if(direction == PopoverArrowDirectionLeft)
    {
        rect.size.width = w - ah - 2*b;
        rect.size.height = h - 2*b;
        rect.origin.x = ah + b;
    ...
    //right arrow
    if(direction == PopoverArrowDirectionRight)
    {
    ...
    //down arrow
    if(direction == PopoverArrowDirectionDown)
    {
    ...
    //left arrow
    if(direction == PopoverArrowDirectionLeft)
    {
...
-(CGGradientRef)createGradient
{
    ...
    if(self.tint == PopoverBlackTint)
    {
        if(_arrowDirection == PopoverArrowDirectionUp)
        {
    ...
    else if(self.tint == PopoverLightGrayTint)
    {
        if(_arrowDirection == PopoverArrowDirectionUp)
        {
    ...
    else if(self.tint == PopoverRedTint)
    {
        if(_arrowDirection == PopoverArrowDirectionUp)
        {
    ...
    else if(self.tint == PopoverGreenTint)
    {
        if(_arrowDirection == PopoverArrowDirectionUp)
        {
...
-(void)setupViews
{
    ...
    else if(_arrowDirection == PopoverArrowDirectionDown)
    {
    ...
    else if(_arrowDirection == PopoverArrowDirectionRight)
    {
        contentRect.origin = CGPointMake(10, 40);        
    ...
    else if(_arrowDirection == PopoverArrowDirectionLeft)
    {
        contentRect.origin = CGPointMake(10 + POPOVER_ARROW_HEIGHT, 40);
        contentRect.size = CGSizeMake(self.bounds.size.width-40, self.bounds.size.height-50);
        _titleLabel.frame = CGRectMake(10, 10, self.bounds.size.width-20, 20);           
    }
