...
- (void)updatePosition {
    ...
    if(string) {
        UIFont *font = self.stringLabel.font;//跟label的字体大小一样
        CGSize size = self.stringLabel.frame.size;//跟label的宽设置一样
        
        NSDictionary * dic = [NSDictionary dictionaryWithObjectsAndKeys:font, NSFontAttributeName,nil];
            
        CGSize stringSize =[string boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin |NSStringDrawingUsesFontLeading attributes:dic context:nil].size;
        
//        CGSize stringSize = [string sizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(200, 300)];//ios7以上摒弃
