...
@interface PopoverController(Private)
...
-(void)addObservers
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(willPresentNewPopover:) name:@"NewPopoverPresented" object:nil];
...
-(id)initWithViewController:(UIViewController*)viewController
{
    ...
    if(self)
    {
        self.arrowDirection = PopoverArrowDirectionAny;
        self.view.userInteractionEnabled = YES;
        _touchView = [[TouchView alloc] initWithFrame:self.view.bounds];
        ...
        _contentView = [[PopoverView alloc] initWithFrame:CGRectMake(0, 0,
...
- (void)viewDidLoad
{
    ...
    [_contentView setArrowDirection:PopoverArrowDirectionUp];
    [_contentView addContentView:_viewController.view];
...
-(void)presentPopoverFromPoint:(CGPoint)fromPoint
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NewPopoverPresented" object:self];
}

-(CGPoint)originFromView:(UIView*)fromView
{
    CGPoint p;
    if([_contentView arrowDirection] == PopoverArrowDirectionUp)
    {
        p.x = fromView.frame.origin.x + fromView.frame.size.width/2.0;
        p.y = fromView.frame.origin.y + fromView.frame.size.height;
    }
    else if([_contentView arrowDirection] == PopoverArrowDirectionDown)
    {
        p.x = fromView.frame.origin.x + fromView.frame.size.width/2.0;
        p.y = fromView.frame.origin.y;        
    }
    else if([_contentView arrowDirection] == PopoverArrowDirectionLeft)
    {
        p.x = fromView.frame.origin.x + fromView.frame.size.width;
        p.y = fromView.frame.origin.y + fromView.frame.size.height/2.0;
    }
    else if([_contentView arrowDirection] == PopoverArrowDirectionRight)
    {
...
-(CGRect)bestArrowDirectionAndFrameFromView:(UIView*)v
{
    ...
    PopoverArrowDirection bestDirection;
    
    //if the user wants vertical arrow, check if the content will fit vertically 
    if(PopoverArrowDirectionIsVertical(self.arrowDirection) ||
       (self.arrowDirection == PopoverArrowDirectionAny && best_h >= best_w))
    {

        //ok, will be vertical
        if(ht == best_h || self.arrowDirection == PopoverArrowDirectionDown)
        {
            //on the top and arrow down
            bestDirection = PopoverArrowDirectionDown;
        ...
        else
        {
            ...
            bestDirection = PopoverArrowDirectionUp;
    ...
    else 
    {
        ...
        if(wl == best_w || self.arrowDirection == PopoverArrowDirectionRight)
        {
            //on the left and arrow right
            bestDirection = PopoverArrowDirectionRight;
        ...
        else
        {
            ...
            bestDirection = PopoverArrowDirectionLeft;
